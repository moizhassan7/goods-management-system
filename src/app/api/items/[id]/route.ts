...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        const depCheck = await checkMasterDataDependencies('item', itemId);
        if (depCheck.notFound) {
            return NextResponse.json({ error: 'Item not found.' }, { status: 404 });
        }
        if (!depCheck.canDelete) {
            return NextResponse.json({
                error: `Cannot delete item "${depCheck.entityName}" because it is linked to ${depCheck.totalCount} active record(s).`,
                ...depCheck,
            }, { status: 409 });
        }
