    ...
    const fetchPendingApprovals = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setDeliveries(data);
                if (data.length > 0) {
                    showNotification('info', 'Data Loaded', `${data.length} deliveries require your approval.`);
                }
            } else {
    ...
    const handleApproval = async (deliveryId: number, action: 'APPROVED' | 'REJECTED') => {
        ...
        try {
            const response = await fetch(`/api/deliveries/${deliveryId}`, { 
                ...
                body: JSON.stringify({ action, approvedBy: 'Current User' }),
            });
            ...
            if (response.ok) {
                ...
                // If approved, update the Approved table/report instantly (if dates match)
                ...
                showNotification(
                    'success', 
                    `${action === 'APPROVED' ? 'Approval' : 'Rejection'} Successful`,
                    `Shipment ID ${result.delivery.shipment_id} has been successfully ${action.toLowerCase()}.`
                );
            } else {
                const errorData = await response.json();
                    ...
                    <CardContent className="p-0">
                        {totalPendingCount === 0 ? (
                            <div className="text-center py-20 px-4">
                                ...
                                <p className="text-gray-500 text-lg">No pending deliveries requiring financial approval at this time.</p>
                                                ...
                                                <TableCell>
                                                    <div className="flex gap-3 justify-center">
                                                        ...
                                                        <AlertDialog>
                                                            ...
                                                            <AlertDialogContent>
                                                                <AlertDialogHeader>
                                                                    <AlertDialogTitle className="text-xl font-bold text-green-700">Confirm Approval</AlertDialogTitle>
                                                                    <AlertDialogDescription className='text-gray-600'>
                                                                        Are you absolutely sure you want to approve the delivery for Shipment **{delivery.shipment_id}**?
                                                                        ...
                                                                        <p className="mt-4 font-medium text-sm text-red-500">
                                                                            This action finalizes the financial transaction and cannot be easily reversed.
                                                                        </p>
